  ...
  return (
    <section className="py-20 bg-gradient-to-br from-slate-50 to-slate-100 relative overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-5">
        <div className="absolute top-10 left-1/4 w-36 h-36 bg-jotah-gold rounded-full animate-pulse blur-xl"></div>
        <div className="absolute bottom-10 right-1/4 w-28 h-28 bg-amber-200 rounded-2xl rotate-45 animate-float blur-lg"></div>
        <div className="absolute top-1/2 left-10 w-20 h-20 bg-orange-200 rounded-full animate-bounce-slow blur-md"></div>
        <div className="absolute top-20 right-10 w-16 h-16 border-2 border-amber-300 rounded-full animate-spin-slow"></div>
      </div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-slate-600">Experience speaks louder than words</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((testimonial) => (
            <div key={testimonial.id} className="bg-white p-6 rounded-2xl shadow-xl border border-slate-200 hover:shadow-2xl hover:transform hover:scale-105 transition-all duration-300">
              <StarRating />
